  ...
  <div class="test-container sma-math-test exam-math-theme">
    ...
    <header class="test-header">
      ...
      <div class="header-title">
        ...
        <p>TKA SMA - Sosiologi (Paket {{ selectedPackage }})</p>
      </div>
...
import { questionPackages } from '../../../data/sma/sosiologi/fulltest'

const route = useRoute()
const router = useRouter()

const packageNumbers = Object.keys(questionPackages).map(Number)
const requestedPackage = Number(route.query.package)
const isTrial = route.query.mode === 'trial'
const selectedPackage = isTrial
  ? 'Trial'
  : packageNumbers.includes(requestedPackage)
    ? requestedPackage
    : packageNumbers[Math.floor(Math.random() * packageNumbers.length)]
const allQuestions = Object.values(questionPackages).flat()
const selectedQuestions = isTrial
  ? [...allQuestions].sort(() => Math.random() - 0.5).slice(0, 10)
  : questionPackages[selectedPackage]
...
const submitTest = () => {
  ...
  router.push('/result?subject=sosiologi&level=sma')
