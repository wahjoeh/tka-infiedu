  ...
  <div class="test-container sma-english-test exam-math-theme">
    ...
    <header class="test-header">
      ...
      <div class="header-title">
        ...
        <p>TKA SMP - Bahasa Inggris ({{ selectedPackage }})</p>
      </div>
...
import fulltestQuestions from '../../../data/smp/english/fulltest'

const route = useRoute()
const router = useRouter()

const isTrial = route.query.mode === 'trial'
const PACKAGE_DIFFICULTY_COUNTS = {
  easy: [12, 11, 11, 11],
  medium: [10, 12, 12, 12],
  hard: [3, 2, 2, 2]
}
...
const buildPackages = () => {
  ...
  Object.entries(PACKAGE_DIFFICULTY_COUNTS).forEach(([difficulty, quotas]) => {
    const questionsForDifficulty = fulltestQuestions.filter(question => question.difficulty === difficulty)
    let offset = 0

    quotas.forEach((quota, packageIndex) => {
      packages[packageIndex].push(...questionsForDifficulty.slice(offset, offset + quota))
      offset += quota
    })
  })
...
const selectedPackageIndex = Math.floor(Math.random() * packages.length)
const selectedPackage = isTrial ? 'Trial' : `Paket ${selectedPackageIndex + 1}`
const questions = (isTrial
  ? [...fulltestQuestions].sort(() => Math.random() - 0.5).slice(0, 10)
...
const submitTest = () => {
  ...
  router.push({
    path: '/result',
    query: { subject: 'english', level: 'smp' }
  })
